...
ArgsReaderActionSetValue :: struct {
	data : rawptr,
	type : typeid,
	parser: proc(str: string, data: rawptr) -> bool,
}
...
args_read :: proc(args: []string, configs : ..ArgsReaderConfig) -> bool {
	count := cast(i32)len(args)
	rctx : ArgsReaderContext
	idx :i32= 1
	ite_args: for idx < count {
		arg := args[idx]
		for &config in configs {
			if ok, after := _apply(args, &config, &idx, &rctx); ok {
				if after == nil {
...
arga_set :: proc(data: ^$T) -> ArgsReaderAction {
	return ArgsReaderActionSetValue{data,T, nil}
}
arga_setp :: proc(data: ^$T, parser: proc(str: string, data: ^T) -> bool) -> ArgsReaderAction {
	return ArgsReaderActionSetValue{data,T, auto_cast parser}
}

@(private="file")
_apply :: proc(args: []string, using config: ^ArgsReaderConfig, arg_idx: ^i32, rctx: ^ArgsReaderContext) -> ( bool, AfterApplyRule ) {
	arg := args[arg_idx^]
	switch r in rule {
	case ArgsRuleIs:
		if r.text == arg {
	...
	case ArgsRuleFollowBy:
		if r.text == arg {
			...
			if r.count == ARGR_FOLLOW_TILL_END {
				count = len(args) - cast(int)(arg_idx^)
			} else if r.count == ARGR_FOLLOW_FALLBACK {
				return true, AfterApplyRule_SetFallbackAction{action}
			} 
			for c in 0..<count {
				if arg_idx^ >= auto_cast len(args) { return false, nil }
				succ := _action(&action, args[arg_idx^])
				arg_idx^ += 1
